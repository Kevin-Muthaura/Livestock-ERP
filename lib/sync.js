    ...
    for (const item of pending) {
      try {
        ...
        if (table === 'notifications') {
          // Notifications are generated server-side by generate_notifications();
          // the client only ever toggles read_status on an existing row by id.
          ({ error } = await supabase
            .from('notifications')
            .update({ read_status: payload.read_status, read_at: payload.read_at })
            .eq('id', payload.id));
        } else {
