...
//
// IMPORTANT: the domain part must be a real, resolvable domain. Supabase's
// email validator checks the domain against real TLD rules, and made-up
// domains like "something.local" (or .test / .example / .invalid) get
// rejected with "Email address ... is invalid", because none of those are
// real, delegated top-level domains. Using the app's own live domain
// (e.g. your-app.vercel.app) sidesteps that entirely, since it's guaranteed
// to be a real, valid domain — the same one this code is already running on.
export function normalizePhone(phone) {
  return phone.replace(/[^\d]/g, ''); // digits only, e.g. 254712345678
}
function pseudoEmailDomain() {
  if (typeof window !== 'undefined' && window.location?.hostname && window.location.hostname !== 'localhost') {
    return window.location.hostname;
  }
  // Local dev fallback (npm run dev on localhost) — Supabase will reject
  // this the same way, so local auth testing should be done against a
  // deployed preview URL, not localhost.
  return 'localhost';
}
function pseudoEmail(phone) {
  return `${normalizePhone(phone)}@${pseudoEmailDomain()}`;
}
...
export async function createAccount(phone, pin) {
  ...
  if (error) {
    if (error.code === 'email_address_invalid' || /invalid/i.test(error.message || '')) {
      throw new Error(
        `This device's web address ("${pseudoEmailDomain()}") wasn't accepted. This usually only happens when testing on localhost — open the app at its real published address (e.g. your-app.vercel.app) and try again.`
      );
    }
    throw error;
  }
